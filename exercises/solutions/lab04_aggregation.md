...
db.subscriptions_practice.insertMany([
  { userId: 'U1', plan: 'pro', price: 40, startedAt: ISODate('2025-01-05'), renewedAt: ISODate('2025-02-05'), canceledAt: null },
  { userId: 'U2', plan: 'basic', price: 15, startedAt: ISODate('2025-01-10'), renewedAt: ISODate('2025-02-10'), canceledAt: ISODate('2025-02-20') }
]);
...
const mrrPipeline = [
  ...
  },
  {
    $addFields: {
      retainedPercent: {
        $cond: [{ $eq: ['$activeUsers', 0] }, 0, { $multiply: [{ $divide: ['$activeUsers', { $add: ['$activeUsers', '$churnedUsers'] }] }, 100] }]
      }
    }
  },
  { $merge: { into: 'subscriptions_reports', on: '_id', whenMatched: 'replace', whenNotMatched: 'insert' } }
];
        ...
        $map: {
          ...
          in: {
            $map: {
              ...
              as: 'j',
              in: { first: { $arrayElemAt: ['$items', '$$i'] }, second: { $arrayElemAt: ['$items', '$$j'] } }
            }
          }
        }
      }
...
db.uptime_practice.aggregate([
  ...
  { $match: { alert: true } },
  {
    $project: {
      ...
      latencyP95: 1,
      mitigation: {
        $cond: [{ $lt: ['$uptimePercent', 99.5] }, 'Investigate outages / restart service', 'Investigate latency spike']
      }
    }
  },
  { $merge: { into: 'uptime_alerts', on: ['service', 'region', 'window'], whenMatched: 'replace', whenNotMatched: 'insert' } }
]);
```
