...
- **Booking workflow:**
  ```javascript
  const session = db.getMongo().startSession({ readConcern: { level: 'majority' }, writeConcern: { w: 'majority' } });
...
- **Workload simulator:**
  ...
  for (let i = 0; i < 10000; i++) {
    db.orders.find({ customerId: `C${i % 1000}` }).hint({ customerId: 1, orderDate: -1 }).toArray();
