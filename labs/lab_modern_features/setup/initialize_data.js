...
async function seedChangeStreamCollections(db) {
  ...
  await events.insertMany([
    ...
    { event: 'page_view', metadata: { path: '/docs/change-streams' }, timestamp: new Date(now.getTime() - 2000) },
  ...
  await users.insertMany([
    ...
    { username: 'sarah_connor', email: 'sarah@example.com', lastLogin: new Date(now.getTime() - 7200000) }
  ]);
...
async function seedTimeSeriesCollections(db) {
  ...
  await db.collection(sensorName).drop().catch(() => {});
...
async function seedAtlasSearchCollections(db) {
  ...
  await articles.insertMany([
    ...
    },
    {
      ...
      author: 'Emily Davis',
      content: 'Explore how MongoDB Atlas Vector Search enables similarity search for AI applications...',
      category: 'AI/ML',
...
async function seedVectorCollections(db) {
  ...
  await products.insertMany([
    ...
    },
    {
      ...
      description: 'Rugged action cam supporting 4K60, waterproof up to 10m, and Gyro stabilization.',
...
async function seedChartsDataset(client) {
  ...
  await sales.insertMany([
    { date: new Date('2024-01-01'), region: 'NA', product: 'Laptop Pro 15', channel: 'Online', units: 42, revenue: 79800 },
    { date: new Date('2024-01-01'), region: 'EU', product: 'Laptop Pro 15', channel: 'Retail', units: 25, revenue: 47500 },
    { date: new Date('2024-01-08'), region: 'NA', product: '4K Monitor', channel: 'Online', units: 60, revenue: 29940 },
    { date: new Date('2024-01-08'), region: 'APAC', product: 'Smart Speaker', channel: 'Resellers', units: 75, revenue: 18750 }
  ]);
}
...
async function seedGridFS(db) {
  ...
  await db.collection(`${GRIDFS_BUCKET}.files`).drop().catch(() => {});
  await db.collection(`${GRIDFS_BUCKET}.chunks`).drop().catch(() => {});
