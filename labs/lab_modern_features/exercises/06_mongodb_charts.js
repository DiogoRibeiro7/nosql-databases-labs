...
class MongoDBChartsExercises {
  ...
  async prepareSalesData() {
    ...
    const dailyRevenue = await sales.aggregate([
      {
        $group: {
    ...
    console.log('\n2. Revenue by Region:');
    const regionRevenue = await sales.aggregate([
      {
        $group: {
          _id: '$region',
          revenue: { $sum: '$revenue' },
    ...
    regionRevenue.forEach(region => {
      const percentage = (region.revenue / salesData.reduce((sum, s) => sum + s.revenue, 0) * 100).toFixed(1);
  ...
  async prepareMetricsData() {
    ...
    const currentMetrics = await metrics.aggregate([
      { $sort: { timestamp: -1 } },
      {
        $group: {
          _id: '$server',
    ...
    currentMetrics.forEach(server => {
      console.log(`  ${server._id}: CPU ${server.currentCPU.toFixed(1)}%, Memory ${server.currentMemory.toFixed(1)}%`);
    });

    // Multi-line Chart Data (CPU over time)
    console.log('\nMulti-line Chart Data (CPU Trends):');
    const cpuTrends = await metrics.aggregate([
      {
        $match: {
  ...
  async prepareCustomerData() {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const lastPurchase = new Date(Math.max(registrationDate.getTime(), Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000));
    ...
    customerData.forEach(customer => {
      customer.averageOrderValue = customer.totalPurchases > 0 ?
    ...
    const scatterData = await customers.aggregate([
      {
        $project: {
          totalPurchases: 1,
          totalSpent: 1,
    ...
    const geoData = await customers.aggregate([
      {
        $group: {
          _id: '$country',
          customers: { $sum: 1 },
    ...
    geoData.forEach(country => {
      console.log(`  ${country._id}: ${country.customers} customers, $${country.totalRevenue.toFixed(0)} revenue`);
    });
  }
  ...
  async prepareFinancialData() {
    ...
    const candlestick = await financial.aggregate([
      { $match: { symbol: 'TECH' } },
      { $sort: { date: -1 } },
      { $limit: 30 },
      {
    ...
    console.log(`  Latest: ${candlestick[0].date} - O:${candlestick[0].open} H:${candlestick[0].high} L:${candlestick[0].low} C:${candlestick[0].close}`);
  ...
  async prepareIoTData() {
    ...
    const tempTrends = await iot.aggregate([
    ...
    // Alert Summary
    console.log('\nAlert Summary:');
    const alerts = await iot.aggregate([
      { $match: { status: 'alert' } },
      {
        $group: {
          _id: {
  ...
  async cleanup() {
    ...
    const collections = [
      'sales_data', 'server_metrics_charts',
      'customer_analytics', 'financial_data',
      'iot_sensors'
    ];
