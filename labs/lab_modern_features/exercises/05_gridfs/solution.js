...
- Backup storage
  ...
  async listFiles() {
    ...
    const jsonFiles = await this.bucket.find({
    ...
    const largeFiles = await this.bucket.find({
      length: { $gt: 1024 * 1024 }
    }).toArray();
      ...
      await new Promise((resolve, reject) => {
        downloadStream
          ...
          .on('finish', () => {
            ...
            const downloadedHash = crypto.createHash('md5')
    ...
    if (csvFile) {
      ...
      await new Promise((resolve) => {
        downloadStream.on('data', (chunk) => {
          ...
          lines.forEach(line => {
            lineCount++;
            if (lineCount > 1 && line.trim()) { // Skip header
  ...
  async fileVersioning() {
    ...
    // List all versions
    console.log('\nAll versions:');
    const versions = await this.bucket.find({ filename })
  ...
  async imageThumbnails() {
    ...
    // List all thumbnails
    console.log('\nThumbnails:');
    const thumbnails = await this.bucket.find({
    ...
    thumbnails.forEach(thumb => {
      ...
      console.log(`    Dimensions: ${thumb.metadata.dimensions.width}x${thumb.metadata.dimensions.height}`);
    });
  }
  ...
  async gridfsStats() {
    ...
    if (files.length > 0) {
      ...
      stats.largestFile = files.reduce((max, file) =>
        file.length > (max?.length || 0) ? file : max, null);
      stats.smallestFile = files.reduce((min, file) =>
        file.length < (min?.length || Infinity) ? file : min, null);
    }
    ...
    if (stats.largestFile) {
      console.log(`  Largest File: ${stats.largestFile.filename} (${(stats.largestFile.length / 1024 / 1024).toFixed(2)} MB)`);
    }
    if (stats.smallestFile) {
      console.log(`  Smallest File: ${stats.smallestFile.filename} (${(stats.smallestFile.length / 1024).toFixed(2)} KB)`);
    }
  ...
  async cleanup() {
    ...
    await this.db.collection('files.files').drop().catch(() => {});
    await this.db.collection('files.chunks').drop().catch(() => {});
