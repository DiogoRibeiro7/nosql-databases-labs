...
async function runAggregations() {
  ...
  try {
    ...
    const ratingsByGenre = await db.collection('movies').aggregate([
    ...
    const topDirectors = await db.collection('movies').aggregate([
    ...
    const moviesByYear = await db.collection('movies').aggregate([
    ...
    const popularCast = await db.collection('movies').aggregate([
    ...
    const revenueByGenre = await db.collection('movies').aggregate([
    ...
    const userPatterns = await db.collection('users').aggregate([
    ...
    userPatterns.forEach(doc => {
      console.log(`${doc.favorite_genre}: ${doc.user_count} users (avg ${doc.avg_movies_watched} movies watched)`);
    });
    ...
    const theaterStats = await db.collection('theaters').aggregate([
      // Add screening count
      { $addFields: {
        screening_count: {
          $cond: {
    ...
    const ratingVariance = await db.collection('movies').aggregate([
    ...
    ratingVariance.forEach(doc => {
      console.log(`${doc.title} (${doc.year}): IMDb ${doc.imdb_rating} vs Tomatoes ${doc.tomatoes_rating} (diff: ${doc.difference})`);
    });
