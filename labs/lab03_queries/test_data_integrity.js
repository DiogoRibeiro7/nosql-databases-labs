...
class DataIntegrityTester {
            ...
            for (const [field, expectedType] of Object.entries(requiredFields)) {
                if (!(field in movie)) {
                    ...
                    if (actualType !== expectedType) {
                        schemaViolations++;
                        console.error(`  ❌ Movie "${movie.title}" field ${field} has wrong type: ${actualType}, expected ${expectedType}`);
                    }
                }
            }
    ...
    async testArrayFields() {
        ...
        movies.forEach(movie => {
            ...
            if (movie.directors) {
                ...
                if (new Set(movie.directors).size !== movie.directors.length) arrayFieldStats.directors.duplicates++;
        ...
        for (const [field, stats] of Object.entries(arrayFieldStats)) {
            ...
            if (stats.empty === 0 && stats.duplicates === 0 && stats.invalid === 0) {
                ...
                this.assert(
                    stats.invalid === 0,
                    `${field} contains ${stats.invalid} invalid entries`
                );
            }
        }
    }
    ...
    async testDuplicates() {
        ...
        const duplicateTitles = await this.db.collection('movies').aggregate([
            {
                $group: {
        ...
        if (duplicateTitles.length === 0) {
            ...
            duplicateTitles.forEach(dup => {
                console.warn(`  ⚠️ Duplicate: "${dup._id.title}" (${dup._id.year}) appears ${dup.count} times`);
            });
            this.results.warnings.push(`Found ${duplicateTitles.length} duplicate title-year combinations`);
        }
    ...
    async calculateDataQualityMetrics() {
        ...
        console.log('     Top Genres:', Object.entries(genreDistribution)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 5)
            .map(([genre, count]) => `${genre}(${count})`)
            .join(', '));
        console.log('     Rating Distribution:', Object.entries(ratingDistribution)
    ...
    calculateQualityScore() {
        ...
        const scores = {
            schemaCompliance: this.results.failed.filter(f => f.includes('schema')).length === 0 ? 100 : 50,
            completeness: Object.values(this.results.dataQualityMetrics)
                .filter(k => typeof k === 'number')
                .reduce((a, b) => a + b, 0) /
                Object.values(this.results.dataQualityMetrics).filter(k => typeof k === 'number').length,
            uniqueness: this.results.warnings.filter(w => w.includes('duplicate')).length === 0 ? 100 : 70,
