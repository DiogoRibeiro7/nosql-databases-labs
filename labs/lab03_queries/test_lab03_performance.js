...
class PerformanceTester {
    ...
    async benchmark(name, setupFn, testFn, teardownFn, expectedMax) {
        ...
        try {
            ...
            if (expectedMax && stats.median > expectedMax) {
                console.log(`  ❌ FAILED: Median time ${stats.median.toFixed(2)}ms exceeds threshold ${expectedMax}ms`);
                this.results.failed++;
                stats.passed = false;
            } else {
    ...
    generateReport() {
        ...
        const fastestQuery = this.results.benchmarks.reduce((min, b) =>
            b.avg < min.avg ? b : min
        );
        const slowestQuery = this.results.benchmarks.reduce((max, b) =>
            b.avg > max.avg ? b : max
        );
    ...
    try {
        ...
        await tester.benchmark(
            ...
            async () => {
                ...
                for (let i = 0; i < 10; i++) {
                    promises.push(
                        tester.movies.find({ year: 2010 + i }).toArray()
                    );
                }
        ...
        console.log(JSON.stringify({
            ...
            averageTime: report.benchmarks.reduce((sum, b) => sum + b.avg, 0) / report.benchmarks.length,
