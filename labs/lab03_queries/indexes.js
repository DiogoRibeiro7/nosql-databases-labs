...
async function createIndexes() {
  ...
  try {
    ...
    let explain = await db.collection('movies')
      .find({ genres: "Action" })
      .explain('executionStats');
    console.log(`Query 1 (genres) - Execution Time: ${explain.executionStats.executionTimeMillis}ms`);
    console.log(`  Stage: ${explain.executionStats.executionStages.stage}`);

    // Query 2: Find movies by year and rating (BEFORE index)
    explain = await db.collection('movies')
      .find({ year: 2015, "imdb.rating": { $gt: 7.0 } })
      .explain('executionStats');
    console.log(`Query 2 (year+rating) - Execution Time: ${explain.executionStats.executionTimeMillis}ms`);
    console.log(`  Stage: ${explain.executionStats.executionStages.stage}`);
    ...
    explain = await db.collection('movies')
      ...
      .explain('executionStats');
    console.log(`Query 3 (director) - Execution Time: ${explain.executionStats.executionTimeMillis}ms`);
    console.log(`  Stage: ${explain.executionStats.executionStages.stage}`);
