...
async function validateUniqueConstraints(db) {
  ...
  customers.forEach(customer => {
    if (emailMap[customer.email]) {
      duplicateEmails++;
      logResult('fail', 'Unique Constraint',
        `Duplicate email: ${customer.email}`,
...
async function validateReferentialIntegrity(db) {
  ...
  orders.forEach(order => {
    if (!customerIdSet.has(order.customer_id)) {
      orphanedOrders++;
      logResult('fail', 'Referential Integrity',
    ...
    if (order.items && Array.isArray(order.items)) {
      order.items.forEach(item => {
        if (!productIdSet.has(item.product_id)) {
          orphanedItems++;
          logResult('warning', 'Referential Integrity',
  ...
  if (orphanedReviews === 0) {
    logResult('pass', 'Referential Integrity', 'All reviews reference valid products and customers');
  }
}
  ...
  orders.forEach(order => {
    if (order.items && Array.isArray(order.items)) {
      ...
      if (Math.abs(order.total_amount - calculatedTotal) > 0.01) {
        inconsistentTotals++;
        logResult('fail', 'Data Consistency',
          `Order ${order.order_id} total mismatch`,
          {
    ...
    if (order.items && Array.isArray(order.items)) {
      order.items.forEach(item => {
        ...
        if (currentProduct && item.product_name !== currentProduct.name) {
          nameMismatches++;
          logResult('warning', 'Data Consistency',
...
async function validateRequiredFields(db) {
  ...
  for (const [collection, fields] of Object.entries(requiredFields)) {
    ...
    documents.forEach(doc => {
      fields.forEach(field => {
        if (doc[field] === undefined || doc[field] === null) {
          missingFields++;
          logResult('fail', 'Required Fields',
    ...
    if (missingFields === 0) {
      logResult('pass', 'Required Fields',
        `All documents in ${collection} have required fields`
      );
    }
  }
}
...
async function validateDataTypes(db) {
  ...
  reviews.forEach(review => {
    if (typeof review.rating !== 'number' || review.rating < 1 || review.rating > 5) {
      invalidRatings++;
      logResult('fail', 'Data Type',
        `Invalid rating for review: ${review.rating}`,
...
async function validateBusinessRules(db) {
  ...
  orders.forEach(order => {
    if (!order.items || order.items.length === 0) {
      emptyOrders++;
      logResult('fail', 'Business Rule',
        `Order ${order.order_id} has no items`
      );
    }
  });
  ...
  orders.forEach(order => {
    ...
    if (orderDate > now) {
      futureDates++;
      logResult('warning', 'Business Rule',
...
async function validateModelingDecisions(db) {
  ...
  orders.forEach(order => {
    if (!order.items || !Array.isArray(order.items)) {
      embeddedItems = false;
      logResult('fail', 'Modeling Decision',
        `Order ${order.order_id} doesn't have embedded items array`
      );
    } else {
      // Check that items contain denormalized product data
      const hasProductData = order.items.every(item =>
        item.product_name && item.unit_price !== undefined
      );
      if (!hasProductData) {
        logResult('warning', 'Modeling Decision',
...
function generateReport() {
  ...
  const passRate = totalTests > 0
    ? (validationResults.passed.length / totalTests * 100).toFixed(1)
    : 0;
