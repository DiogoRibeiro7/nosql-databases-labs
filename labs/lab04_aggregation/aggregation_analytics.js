...
async function runAnalytics() {
    ...
    try {
        ...
        const clvAnalysis = await db.collection('sales').aggregate([
            {
                $group: {
                    _id: '$customer_id',
        ...
        clvAnalysis.forEach((customer, index) => {
            ...
            console.log(`   Active for: ${Math.round(customer.lifetime_days)} days | Frequency: ${customer.purchase_frequency.toFixed(2)} orders/day`);
        });
        ...
        const rfmAnalysis = await db.collection('sales').aggregate([
            {
                $group: {
                    _id: '$customer_id',
                    last_purchase: { $max: '$date' },
            ...
            },
            {
                $project: {
                    ...
                    recency_days: {
                        $divide: [
                            { $subtract: [currentDate, '$last_purchase'] },
                            1000 * 60 * 60 * 24
                        ]
                    },
                    frequency: 1,
        ...
        rfmAnalysis.forEach((segment, index) => {
            ...
            console.log(`${recencyLabel} Customers (${segment._id.min.toFixed(0)}-${segment._id.max.toFixed(0)} days ago):`);
            console.log(`  Count: ${segment.customers.length} | Avg Frequency: ${segment.avg_frequency.toFixed(1)} | Avg Value: $${segment.avg_monetary.toFixed(2)}`);
        });
        ...
        const cohortAnalysis = await db.collection('sales').aggregate([
            {
                $group: {
                    _id: {
            ...
            },
            {
                $project: {
                    ...
                    avg_revenue_per_customer: {
                        $round: [
                            { $divide: ['$total_revenue', { $size: '$customers' }] },
                            2
                        ]
                    }
                }
        ...
        cohortAnalysis.forEach(cohort => {
            console.log(`${cohort.cohort}: ${cohort.customer_count} customers, $${cohort.avg_revenue_per_customer} avg revenue`);
        });
        ...
        const productAffinity = await db.collection('sales').aggregate([
            {
                $group: {
                    _id: '$customer_id',
        ...
        const funnelAnalysis = await db.collection('sales').aggregate([
            {
                $group: {
                    _id: '$customer_id',
                    first_purchase: { $min: '$date' },
        ...
        const seasonalityAnalysis = await db.collection('sales').aggregate([
            {
                $project: {
                    month: { $month: '$date' },
        ...
        const churnAnalysis = await db.collection('sales').aggregate([
            {
                $group: {
                    _id: '$customer_id',
            ...
            },
            {
                $project: {
                    ...
                    days_since_last_order: {
                        $divide: [
                            { $subtract: [currentDate, '$last_order_date'] },
                            1000 * 60 * 60 * 24
                        ]
                    },
                    order_count: 1,
        ...
        const profitabilityAnalysis = await db.collection('sales').aggregate([
            {
                $lookup: {
                    from: 'products',
