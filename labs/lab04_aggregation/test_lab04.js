...
class Lab04AggregationTester {
  ...
  async verifyMonthlyRevenue() {
    await this.run('Monthly revenue aggregation', async () => {
      const monthly = await this.sales.aggregate([
        {
          $project: {
      ...
      const total = await this.sales.aggregate([
        { $group: { _id: null, revenue: { $sum: '$total_amount' } } },
      ]).toArray();

      assert(Math.abs(monthlyTotal - total[0].revenue) < 0.01, 'Monthly totals do not match grand total');
    });
  }

  async verifyTopCustomers() {
    await this.run('Top customers aggregation', async () => {
      const topCustomers = await this.sales.aggregate([
        {
          $group: {
            _id: '$customer_id',
      ...
      assert(topCustomers[0].total_spent >= topCustomers[4].total_spent, 'Top customers not sorted properly');
    });
  }

  async verifyProductPerformance() {
    await this.run('Product performance aggregation', async () => {
      const productPerformance = await this.sales.aggregate([
        {
          $group: {
            _id: '$product_id',
  ...
  async verifyCustomerSegmentation() {
    await this.run('Customer segmentation aggregation', async () => {
      const segments = await this.customers.aggregate([
        {
          $lookup: {
            from: 'sales',
  ...
  async verifyDataIntegrity() {
    await this.run('Cross-collection integrity', async () => {
      const orphanSales = await this.sales.aggregate([
        {
          $lookup: {
            from: 'customers',
            localField: 'customer_id',
      ...
      const orphanProducts = await this.sales.aggregate([
        {
          $lookup: {
            from: 'products',
