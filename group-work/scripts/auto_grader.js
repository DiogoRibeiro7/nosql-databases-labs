...
class AutoGrader {
  ...
  async gradeCategory(category, config, submissionPath) {
    ...
    categoryScore.percentage = categoryScore.max_total > 0
  ...
  async gradeFunctionality(submissionPath, criteria) {
    ...
    // Grade correctness
    scores.correctness = {
      points: testResults.passed ? criteria.correctness.max_points :
    ...
    scores.completeness = {
      points: Math.floor(criteria.completeness.max_points * (completedQueries / requiredQueries.length)),
      max_points: criteria.completeness.max_points,
      feedback: `${completedQueries}/${requiredQueries.length} required queries implemented`
    };

    // Grade edge cases
    const edgeCaseTests = testResults.tests?.filter(t => t.name.includes('edge') || t.name.includes('boundary')) || [];
    const edgeCasePass = edgeCaseTests.filter(t => t.passed).length;

    scores.edge_cases = {
      points: edgeCaseTests.length > 0
      ...
      feedback: edgeCaseTests.length > 0
  ...
  async gradeCodeQuality(submissionPath, criteria) {
    ...
    const hasTests = fs.existsSync(path.join(submissionPath, 'tests')) ||
                    fs.existsSync(path.join(submissionPath, 'test'));

    scores.organization = {
      points: (hasProperStructure ? criteria.organization.max_points * 0.5 : 0) +
    ...
    const bestPractices = {
      ...
      noHardcodedSecrets: !jsFiles.some(f => /password|secret|key\s*=\s*["']/.test(fs.readFileSync(f, 'utf8'))),
      hasErrorHandling: jsFiles.some(f => fs.readFileSync(f, 'utf8').includes('try'))
    };

    const practiceScore = Object.values(bestPractices).filter(v => v).length / Object.keys(bestPractices).length;
    ...
    scores.best_practices = {
      ...
      feedback: `Practices: ${Object.entries(bestPractices).map(([k, v]) => `${k}: ${v ? '✓' : '✗'}`).join(', ')}`
    };
  ...
  async gradePerformance(submissionPath, criteria) {
    ...
    const efficiencyScore = Object.values(perfChecks).filter(v => v).length / Object.keys(perfChecks).length;
    ...
    scores.query_efficiency = {
      ...
      feedback: `Optimizations: ${Object.entries(perfChecks).filter(([k, v]) => v).map(([k]) => k).join(', ') || 'None found'}`
    };
  ...
  async gradeTesting(submissionPath, criteria) {
    ...
    scores.test_quality = {
      points: (hasAssertions ? criteria.test_quality.max_points * 0.5 : 0) +
  ...
  async gradeDocumentation(submissionPath, criteria) {
    ...
    scores.completeness = {
      points: Math.floor(criteria.completeness.max_points * (foundSections.length / requiredSections.length)),
      max_points: criteria.completeness.max_points,
